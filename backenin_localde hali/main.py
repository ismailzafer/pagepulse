...
app = FastAPI(title="PagePulse API", description="PDF to Text Conversion Service (Local)")
...
@app.get("/")
async def read_root():
    return {"message": "Welcome to PagePulse API (Local)"}
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {
            "status": "success",
            "file_id": file_id,
            "message": "PDF uploaded and converted successfully (local)"
        }
...
async def get_conversion_status(file_id: str):
    """
    Check the status of a PDF conversion (local)
    """
    try:
        txt_path = os.path.join(CONVERSION_DIR, f"{file_id}.txt")
...
async def download_text(file_id: str):
    """
    Download the converted text file (local)
    """
    try:
        txt_path = os.path.join(CONVERSION_DIR, f"{file_id}.txt")
